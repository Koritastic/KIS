            ...
            foreach (AvailablePart avPart in PartLoader.LoadedPartsList)
            {
                ...
                for (int i = 0; i < avPart.partPrefab.CrewCapacity; i++)
                {
                    try
                    {
                        ...
                        //KSPDev.Logger.logInfo("Pod inventory module(s) for seat {0} loaded successfully", i);
                    }
                    catch
                    {
                        //KSPDev.Logger.logWarning("Pod inventory module(s) for seat {0} can't be loaded!", i);
                    }
                }
            }
        }
