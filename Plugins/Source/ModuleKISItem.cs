...
namespace KIS
{
    ...
    public class ModuleKISItem : PartModule
    {
        ...
        public virtual void OnPartUnpack()
        {
            ...
            if (staticAttached)
            {
                //KSPDev.Logger.logInfo("Re-attach static object (OnPartUnpack)");
                GroundAttach();
            }
        }
        ...
        public void GroundAttach()
        {
            ...
            //KSPDev.Logger.logInfo("Create fixed joint on the kinematic rigidbody");
            ...
            CurJoint.connectedBody = obj.GetComponent<Rigidbody>();
            fixedJoint = CurJoint;
        ...
        public void GroundDetach()
        {
            if (staticAttached)
            {
                //KSPDev.Logger.logInfo("Removing static rigidbody and fixed joint on: {0}", this.part.partInfo.title);
                if (fixedJoint) Destroy(fixedJoint);
                if (connectedGameObject) Destroy(connectedGameObject);
                fixedJoint = null;
